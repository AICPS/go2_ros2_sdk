...
from action_msgs.msg import GoalInfo, GoalID
...
class Go2DriverNode(Node):
    ...
    def __init__(self, event_loop=None):
        ...
        # Nav2 cancel client (configurable action name)
        self.declare_parameter('nav2_action_name', 'navigate_to_pose')
        nav2_action = self.get_parameter('nav2_action_name').get_parameter_value().string_value
        self._nav2_cancel_client = self.create_client(CancelGoal, f'{nav2_action}/_action/cancel')
        # Optional rate limiting to avoid spamming cancel requests
        self.declare_parameter('manual_cancel_rate_hz', 2.0)
        self._manual_cancel_min_interval = 1.0 / self.get_parameter('manual_cancel_rate_hz').get_parameter_value().double_value
        self._last_manual_cancel = 0.0
    ...
    def _cancel_nav2_goals(self) -> None:
        """Request Nav2 to cancel active goals (non-blocking)."""
        try:
            now = time.time()
            if now - self._last_manual_cancel < self._manual_cancel_min_interval:
                return
            if not self._nav2_cancel_client.wait_for_service(timeout_sec=0.2):
                self.get_logger().debug("Nav2 cancel service not available")
                return
            req = CancelGoal.Request()
            gi = GoalInfo()
            gi.goal_id = GoalID()
            gi.stamp = BuiltinTime(sec=0, nanosec=0)
            req.goal_info = gi
            self._nav2_cancel_client.call_async(req)
            self._last_manual_cancel = now
            self.get_logger().info("Requested Nav2 cancel (via joystick).")
        except Exception as e:
            self.get_logger().error(f"Failed to request Nav2 cancel: {e}")

    def _on_joy(self, msg: Joy) -> None:
        """Callback for joystick - cancel Nav2 on manual input."""
        self.joy_state = msg

        # Simple thresholds for axes/buttons to detect manual intent
        axis_threshold = 0.05
        manual_axes = any(abs(a) > axis_threshold for a in msg.axes) if msg.axes else False
        manual_buttons = any(b for b in msg.buttons) if msg.buttons else False

        if manual_axes or manual_buttons:
